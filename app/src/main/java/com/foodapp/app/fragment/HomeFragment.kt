            ...
            foodAdapter = object : BaseAdaptor<FoodItemModel>(activity!!, foodList!!) {
                ...
                override fun onBindData(
                    ...
                    /* if (SharePreference.getStringPref(activity!!, SharePreference.SELECTED_LANGUAGE)
                         ...
                         tvFoodPriceGrid.text = String.format(Locale.US,"%.2f",foodList!!.get(position).getItem_price()!!.toDouble())+ getCurrancy(activity!!)
                     } else {
                         tvFoodPriceGrid.text = getCurrancy(activity!!)+ String.format(Locale.US,"%.2f",foodList!!.get(position).getItem_price()!!.toDouble())
                     }*/

                    Common.getPrice(
                        foodList!!.get(position).getItem_price()!!.toDouble(),
                        tvFoodPriceGrid,
                        activity!!
                    )
                    if (SharePreference.getBooleanPref(activity!!, isLinearLayoutManager)) {
    ...
    override fun onResume() {
        ...
        if (SharePreference.getBooleanPref(activity!!, SharePreference.isLogin)) {
            if (Common.isCartTrueOut) {
                if (isCheckNetwork(activity!!)) {
                    Common.getLog("isCart",Common.isCartTrueOut.toString())
